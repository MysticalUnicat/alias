...
static alias_ecs_Result _construct_query(alias_ecs_Instance *instance, alias_TransformBundle *bundle, alias_ecs_Query **out, bool translation, bool rotation) {
  ...
  alias_ecs_QueryCreateInfo create_info = {
      .num_write_components = 1, .write_components = &bundle->Transform2D_component, .read_components = read_components, .filters = filters};
...
alias_ecs_Result alias_TransformBundle_initialize(alias_ecs_Instance *instance, alias_TransformBundle *bundle) {
  alias_ecs_register_component(instance, &(alias_ecs_ComponentCreateInfo){.size = sizeof(alias_LocalToWorld2D)}, &bundle->LocalToWorld2D_component);

  alias_ecs_register_component(instance,
                               &(alias_ecs_ComponentCreateInfo){
                                   .size = sizeof(alias_Transform2D), .num_required_components = 1, .required_components = &bundle->LocalToWorld2D_component},
                               &bundle->Transform2D_component);

  alias_ecs_register_component(instance,
                               &(alias_ecs_ComponentCreateInfo){
                                   .size = sizeof(alias_Translation2D), .num_required_components = 1, .required_components = &bundle->Transform2D_component},
                               &bundle->Translation2D_component);

  alias_ecs_register_component(
      instance,
      &(alias_ecs_ComponentCreateInfo){.size = sizeof(alias_Rotation2D), .num_required_components = 1, .required_components = &bundle->Transform2D_component},
      &bundle->Rotation2D_component);

  alias_ecs_register_component(instance, &(alias_ecs_ComponentCreateInfo){.size = sizeof(alias_Parent2D)}, &bundle->Parent2D_component);
  ...
  alias_ecs_create_query(instance,
                         &(alias_ecs_QueryCreateInfo){
                             ...
                             .num_filters = 2,
                             .filters = (alias_ecs_QueryFilterCreateInfo[]){{.component = bundle->Transform2D_component, .filter = ALIAS_ECS_FILTER_MODIFIED},
...
static void _update2d_translation_rotation(void *ud, alias_ecs_Instance *instance, alias_ecs_EntityHandle entity, void **data) {
  ...
  transform->value = alias_pga2d_motor(rotation->value, 1, translation->value);
}
