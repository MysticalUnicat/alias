...
void generate(void) {
  ...
  for(int i = 0; i < num_basis; i++) {
    ...
    *name++ = '\0';
    ...
    basis[i].bits = i;
    basis[i].grade = tabula_bits_popcount_uint32(i);
  }
...
int main(int argc, char *argv[]) {
  struct option longopts[] = {{"positive", tabula_required_argument, NULL, 'p'},   {"negative", tabula_required_argument, NULL, 'n'},
                              {"degenerate", tabula_required_argument, NULL, 'd'}, {"prefix", tabula_required_argument, NULL, 0},
                              {"unary", tabula_required_argument, NULL, 0},        {"binary", tabula_required_argument, NULL, 0},
                              {"code", tabula_required_argument, NULL, 0},         {NULL, tabula_no_argument, NULL, 0}};
  ...
  tabula_getopt_state getopt_s = {0};

  for(;;) {
    int longindex;
    int opt = tabula_getopt_long(argc, argv, "p:n:d:", longopts, &longindex, &getopt_s);
    ...
    case 0:
      ...
      case 3:
        ...
        break;
      case 4:
        _add_op_alias(getopt_s.arg, argv[getopt_s.ind], 0);
        getopt_s.ind++;
        break;
      case 5:
        _add_op_alias(getopt_s.arg, argv[getopt_s.ind], 1);
        getopt_s.ind++;
        break;
      case 6: {
        uint32_t grades = 0, l = strlen(getopt_s.arg);
        for(int i = 1; i < l; i++) {
          grades |= (1 << (getopt_s.arg[i] - '0'));
        }
        _add_code(getopt_s.arg[0], grades);
      }
      default:
        break;
      }
