...
static struct {
  ...
  [alias_memory_Format_Unknown]   = { 0 },
  [alias_memory_Format_Uint8]     = { 1 },
  [alias_memory_Format_Uint16]    = { 2 },
  [alias_memory_Format_Uint32]    = { 4 },
  [alias_memory_Format_Uint64]    = { 8 },
  [alias_memory_Format_Sint8]     = { 1 },
  [alias_memory_Format_Sint16]    = { 2 },
  [alias_memory_Format_Sint32]    = { 4 },
  [alias_memory_Format_Sint64]    = { 8 },
  [alias_memory_Format_Unorm8]    = { 1 },
  [alias_memory_Format_Unorm16]   = { 2 },
  [alias_memory_Format_Snorm8]    = { 4 },
  [alias_memory_Format_Snorm16]   = { 8 },
  [alias_memory_Format_Uscaled8]  = { 1 },
  [alias_memory_Format_Uscaled16] = { 2 },
  [alias_memory_Format_Sscaled8]  = { 4 },
  [alias_memory_Format_Sscaled16] = { 8 },
  [alias_memory_Format_Urgb8]     = { 1 },
  [alias_memory_Format_Float16]   = { 2 },
  [alias_memory_Format_Float32]   = { 4 },
...
#define _WRITES(X, ...) \
  ...
  X(Snorm8,    int8_t,   0,      0,      (uint8_t)(in * 127.0),    ## __VA_ARGS__) \
  X(Snorm16,   int16_t,  0,      0,      (uint16_t)(in * 32767.0), ## __VA_ARGS__) \
...
#define _GEN_CONVERT_1(R_NAME, R_TYPE, R_U, R_S, R_F) \
  _WRITES(_GEN_CONVERT_2, R_NAME, R_TYPE, R_U, R_S, R_F)
    ...
    for(uint32_t i = 0; i < count; i++) {                                                                                                                      \
      ...
      for(uint32_t j = 0; j < length; j++) {                                                                                                                   \
        ALIAS_CPP_IF1(ALIAS_CPP_IS_ZERO(W_S))(                                                                                                                 \
          ALIAS_CPP_IF1(ALIAS_CPP_IS_ZERO(W_U))(                                                                                                               \
            float in = R_F;                                                                                                                                    \
            (void)in;                                                                                                                                          \
            *w = W_F;                                                                                                                                          \
          )(                                                                                                                                                   \
...
#define _CONVERT_FN_2(W_NAME, W_TYPE, W_U, W_S, W_F, R_NAME) [alias_memory_Format_##W_NAME * alias_memory_Format_COUNT + alias_memory_Format_##R_NAME] = _ ## R_NAME ## _to_ ## W_NAME,

static ConvertFn _convert_functions[alias_memory_Format_COUNT * alias_memory_Format_COUNT] = {
  _READS(_CONVERT_FN_1)
};

static inline void _copy(
    void *              dst_pointer
  , uint32_t            dst_stride
  , alias_memory_Format dst_format
  , const void *        src_pointer
  , uint32_t            src_stride
  , alias_memory_Format src_format
  , uint32_t            length
  , uint32_t            count
) {
...
int alias_memory_SubBuffer_write(alias_memory_SubBuffer * dst, uint32_t index, uint32_t count, alias_memory_Format src_format, uint32_t src_stride, const void * src) {
  uint32_t
    length = dst->type_length,
    dst_stride = dst->stride ? dst->stride : _format_info[dst->type_format].size * length;
  ...
  _copy(
      (uint8_t *)dst->pointer + index * dst_stride
    , dst->stride
    , dst->type_format
    , src
    , src_stride
...
int alias_memory_SubBuffer_read(const alias_memory_SubBuffer * src, uint32_t index, uint32_t count, alias_memory_Format dst_format, uint32_t dst_stride, void * dst) {
  uint32_t
    length = src->type_length,
    src_stride = src->stride ? src->stride : _format_info[src->type_format].size * length;
  ...
  _copy(
    ...
    , dst_stride
    , dst_format
    , (uint8_t *)src->pointer + index * dst_stride
    , src_stride
    , src->type_format
    , length
    , count
