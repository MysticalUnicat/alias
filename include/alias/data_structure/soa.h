...
static inline bool SOA_initialize(SOA * soa, alias_MemoryCB * mcb, uint32_t num_columns, const size_t * sizes, const size_t * alignments) {
  soa->length = 0;
  soa->capacity = 0;
...
static inline void SOA_free(SOA * soa, alias_MemoryCB * mcb) {
  for(uint32_t i = 0; i < soa->num_columns; i++) {
...
static inline bool SOA_set_capacity(SOA * soa, alias_MemoryCB * mcb, uint32_t new_capacity) {
  for(uint32_t i = 0; i < soa->num_columns; i++) {
...
static inline bool SOA_space_for(SOA * soa, alias_MemoryCB * mcb, uint32_t count) {
  uint32_t new_capacity = soa->length + count;
  if(new_capacity > soa->capacity) {
    return SOA_set_capacity(soa, mcb, new_capacity);
  }
  return true;
}

static inline uint32_t SOA_push(SOA * soa) {
  return soa->length++;
}

static inline const void * SOA_read(const SOA * soa, uint32_t index, uint32_t column) {
  return index < soa->length ? soa->column_data[column] + index : NULL;
}

static inline void * SOA_write(SOA * soa, uint32_t index, uint32_t column) {
  return index < soa->length ? soa->column_data[column] + index : NULL;
}
